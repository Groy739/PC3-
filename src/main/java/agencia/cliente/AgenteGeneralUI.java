...
public class AgenteGeneralUI extends JFrame {
    ...
    private volatile ObjectOutputStream outPrincipal;
    private volatile ObjectInputStream inPrincipal;
    ...
    private void configurarVentanaPremium() {
        setTitle("Terminal General - PC3 - Rodrigo Ovando");
        setSize(900, 650); 
        ...
        JLabel lblTitulo = new JLabel("MODULO DE SERVICIO GENERAL");
        ...
        txtAgente = crearCampoPro("Agente Asignado:", "Rodrigo Ovando", false); 
        ...
        estilizarBoton(btnLlamar, AZUL_NEON, Color.BLACK, 200);
        ...
        addWindowListener(new java.awt.event.WindowAdapter() {
            ...
            public void windowClosing(java.awt.event.WindowEvent e) {
                if (radarCola != null) radarCola.stop();
    ...
    Thread hilo = new Thread(() -> {
        try {
            ...
            if (outPrincipal == null || inPrincipal == null) {
                SwingUtilities.invokeLater(() -> {
                    JOptionPane.showMessageDialog(this, "ERROR: No hay conexión activa con el servidor.", "Fallo de Red", JOptionPane.ERROR_MESSAGE);
            ...
            outPrincipal.writeObject("SOLICITAR_GENERAL");
            outPrincipal.flush();

            Object respuesta = inPrincipal.readObject();

         
            if (respuesta instanceof agencia.modelos.TicketViaje) {
                ticketActual = (agencia.modelos.TicketViaje) respuesta;
                ...
                SwingUtilities.invokeLater(() -> {
                    ...
                    actualizarEstadoUI("EN ATENCION GENERAL", AZUL_NEON, true);
                ...
                SwingUtilities.invokeLater(() -> {
                    lblTicket.setText("---");
                    lblDpi.setText("---");
                    actualizarEstadoUI("ESPERANDO CLIENTE", COLOR_MUTED, false);
                });
            }
            ...
            SwingUtilities.invokeLater(() -> {
                actualizarEstadoUI("ERROR DE COMUNICACIÓN", ROJO_ALERTA, false);
    ...
    private void finalizarAtencionConcurrente() {
        ...
        Thread hilo = new Thread(() -> {
            try {
                ...
                try (Socket socketFinalizar = new Socket(config.getIpServidor(), config.getPuerto())) {
   ...
   private void iniciarRadarCola() {
    radarCola = new Timer(3000, e -> {
        new Thread(() -> {
            Socket s = null;
            try {
                s = new Socket(config.getIpServidor(), config.getPuerto());
                ObjectOutputStream out = new ObjectOutputStream(s.getOutputStream());
                out.writeObject("TAMANO_COLA_GENERAL");
                out.flush();
                ...
                if (respuesta instanceof Integer) {
                    int cantidad = (int) respuesta;
                    
                    SwingUtilities.invokeLater(() -> {

                        lblEnCola.setText(cantidad > 0 ? "COLA: " + cantidad + " CLIENTES" : "COLA: VACÍA");
                        lblEnCola.setForeground(cantidad > 0 ? AZUL_NEON : COLOR_MUTED);

                        
                        if (ticketActual == null) {
                            btnLlamar.setEnabled(cantidad > 0); 
                        } else {
                            btnLlamar.setEnabled(false);
                        }
                    });
                ...
                try { if (s != null) s.close(); } catch (Exception ex) { ex.printStackTrace(); }
            }
        }).start();
    });
    radarCola.start();
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> new AgenteGeneralUI().setVisible(true));
    }
}